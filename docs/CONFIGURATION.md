...
Flags (all optional):
  ...
  --as, --auth-strategy <strategy>  bearer | apiKey | basic | oauth-cc | custom | none (default: bearer)
...
XERA_BASE_URL=               # optional — overrides web.baseUrl[XERA_ENV] for both
                             #   the Playwright run and the auth-setup browser
                             #   context (so a setupScript using `page.goto('/login')`
                             #   resolves). Honored by `auth-setup`, `exec`'s
                             #   implicit auth-refresh, and `stage-auth`. (v0.20.3)
XERA_HEADED=1                # optional — launch a visible Chromium for `auth-setup`
                             #   and `exec` auth-refresh so a human can complete an
                             #   interactive SSO / MFA challenge once. The
                             #   encrypted session is cached; subsequent runs
                             #   work headless. (v0.20.4)
```

## HTML report (v0.21)

`xera-internal exec` always writes the JSON reporter that `normalize` reads. To **also** produce a Playwright HTML report routed into the run dir, append extra reporters with `--reporter`:

```sh
# All three accepted forms:
npx xera-internal exec XFB-9 --reporter html        # space form
npx xera-internal exec XFB-9 --reporter=html        # equals form (Playwright-style)
npx xera-internal exec XFB-9 --reporter=html,line   # comma-joined multi-reporter
```

When `html` is requested, exec sets `PLAYWRIGHT_HTML_REPORT=<runDir>/html/` and `PW_TEST_HTML_REPORT_OPEN=never` so the run does not block on Playwright's auto-served viewer.

To view it without leaving the `xera` surface:

```sh
npx xera show-report XFB-9                          # serves the latest run
npx xera show-report XFB-9 --run 2026-06-05T11-22-33 # specific historical run
npx xera show-report XFB-9 --host 0.0.0.0 --port 8080
```

Defaults: `127.0.0.1:9323`. Exits non-zero with a clear `--reporter=html` hint if no HTML report exists for the resolved run.

For running Playwright directly (e.g. `npx playwright test --ui`), the spec needs a plaintext `storageState` file. `xera-internal stage-auth` decrypts the cached web session into `.xera/.auth/.cache/<role>.json` — the exact path the generated `spec.ts` references via `test.use({ storageState })`:

```sh
npx xera-internal stage-auth                         # stage every web role
npx xera-internal stage-auth --role admin            # one role only
```
...
- `xera doctor --strict` skips the per-role `http auth file present` checks and reports a single roll-up entry: `strategy 'none' — no per-role auth state required`. Previous versions hard-failed per-role here, blocking `/xera-run` Step 0 (#218).
- `xera-internal auth-setup` short-circuits the http loop: logs `http: skipped (strategy: 'none' — no per-role auth state required)` and never invokes the scaffolded `presetHttpAuth`, which throws for `'none'`. A clean `xera init` mixed project now passes both `auth-setup` and `doctor --strict` out of the box (#220).

The CLI flag list also includes `custom` since v0.20.6:

```sh
xera update --auth-strategy custom   # was rejected by the CLI before #219;
                                     # the core schema and HTTP runtime have
                                     # always supported it.
```
