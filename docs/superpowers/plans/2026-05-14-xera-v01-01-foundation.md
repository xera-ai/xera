...
**Prereqs:** Bun ≥1.1.0 installed. `gh` CLI authenticated.
...
git commit -m "chore: bootstrap bun workspace + biome + tsconfig"
```
...
```json
{
  ...
  "files": ["dist", "bin"],
  "scripts": {
    "build": "bun build ./src/index.ts ./src/bin/internal.ts --outdir ./dist --target bun",
