...
| Where | Symptom | Fix |
|---|---|---|
| Plan 01 / Task 0.2 | `package.json` templates used `"@xera-ai/X": "workspace:*"` | Node's publish-time `workspace:*` substitution lags lockfile by one version, so consumers of published packages couldn't resolve. Switched to explicit caret semver (e.g. `"^0.1.4"`). |
...
| Plan 01 / Task 1.7 | File-lock test used `require('node:os')` | Doesn't work in ESM tests. Switched to `import`. |
...
| Plan 03 / Task 8.1 | the bundler failed on `chromium-bidi` resolution (pulled in via Playwright) | Externalize Playwright + workspace + heavy deps so they resolve at runtime instead of being bundled (now `external: [/^[^./]/]` in tsup; deps stay in package.json). |
...
Initial plan assumed the runtime would resolve workspace imports from `src/` automatically. Reality: package.json `exports` field pointed at `./dist/index.js`, so consumers (including the workspace's own packages depending on each other) needed `dist/` to exist before tests ran.

**Fix (commit `b915156`, `f309777`):** Added a `source` exports condition:
...
Node's runtime picks `source` first → resolves to TS source in monorepo dev. Node consumers pick `import` → resolves to compiled JS. **`src/` must be in `files`** in published `package.json` so the `source` condition still works for consumers running under Node.

### 2. `npm publish` substitution of `workspace:*` lags by one version

When you bump a workspace package and `npm publish`, the version that gets substituted into a sibling's `dependencies` is the **lockfile-recorded version**, not the just-updated `package.json` version. So `cli@0.1.3` ended up depending on `core@0.1.2`, which still had the original `workspace:*` poison.

**Fix:** Use explicit caret semver in all cross-workspace deps. Never `workspace:*`. (Captured in `AGENTS.md § Workspace deps`.)

### 3. `source` exports condition pointing at `src/` requires shipping `src/`

After fix #1, an attempt to drop the `source` condition broke consumers because the bundled `dist/index.js` had unresolvable transitive workspace imports. Adding `src/` to the published `files` array fixed both paths.
...
`init.ts` does `require.resolve('@xera-ai/skills/package.json')` to locate the skill files for copying. Plan didn't list `@xera-ai/skills` as a dependency of `@xera-ai/cli`, so installing the CLI standalone via `npx` failed.
...
- **`xera-report` skill skipped AI classification.** Session would shell out to `npx xera-internal report` without first writing `classifier-input.json`. Rewrote the skill to call out step 4 as "cognitive work YOU must do, not a CLI command" with an explicit "do not skip" guard.
...
- **`@types/node` was in the consumer's tsconfig template but never installed.** Removed `@types/node` from `templates/tsconfig.json.tmpl`.
...
`npx @xera-ai/cli init` is the canonical entry point for new projects.

---

## What we'd do differently next time

1. **Always execute the plan against a clean machine before declaring it done.** Three of the bugs (source condition, `.claude/commands/`, `@types/node`) would have surfaced if we'd done a fresh-clone smoke test before publishing 0.1.0.
