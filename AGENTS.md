...
| **v0.19** | 🚧 | **CONTRACT_DRIFT on web traces** (detection) — `web.spec` config + `resolveOpenApiSpec`; opt-in `xeraNetwork` recorder (`@xera-ai/web`, env-gated by `XERA_NETWORK_LOG`) captures scrubbed page responses to a `network.jsonl` sidecar; `/xera-report` matches each FAIL scenario's calls to **documented** OpenAPI endpoints via the existing `classifyContractDrift` and stamps `CONTRACT_DRIFT` per scenario. Self-heal of CONTRACT_DRIFT (rewrite `spec.ts` assertions) is the Phase-2 follow-up |
