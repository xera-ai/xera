...
import { preflightRefreshReuseWebSession } from './preflight-refresh';
...
export async function execCmd(argv: string[]): Promise<number> {
  ...
  try {
    ...
    // Pre-flight refresh of HTTP auth for `reuse-web-session`: if the http
    // file is within `refreshBuffer` of expiring but the web file is still
    // fresh, re-derive http from web (no browser needed). If web is also
    // expired, fall through — the runtime's "auth expired" error fires with
    // the actionable re-login hint at point of use.
